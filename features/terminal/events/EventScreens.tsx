...
export function Home(props: ScreenProps & { poster: string }) {
  ...
  return <article className="tm-home" data-poster={showPoster}>
    ...
    <div className="tm-home-action tm-cell">
      ...
      <div className="tm-home-actions"><Action page="gate" event={event.id}>{event.status === 'ARCHIVED' ? t('아카이브 보기', 'View archive') : t('이벤트 보기', 'View event')}</Action>{event.status === 'ARCHIVED' && <Link className="tm-text-link" href={href('signal')}><span>{t('다음 이벤트 소식 받기', 'Get future event updates')}</span></Link>}</div>
    </div>
  </article>;
}
...
export function Gate(props: ScreenProps & { poster: string }) {
  ...
  return <>
    ...
    <div className="tm-gate-grid">
      ...
      <section className="tm-gate-lineup tm-cell"><h2 className="tm-eyebrow">LINEUP</h2><ul>{publicArtists.map(artist => <li key={artist.id}><Link href={href('lineup', event.id, artist.id)}><span>{artist.name}</span></Link><p data-motion-copy>{artist.time === 'TBA' ? t('시간 미공개', 'Set time not announced') : artist.time}</p></li>)}</ul>{event.artists.length > publicArtists.length && <p className="tm-gate-note">{t(`그 외 ${event.artists.length - publicArtists.length}개 항목은 아티스트 정보 미공개`, `${event.artists.length - publicArtists.length} other artist records are unpublished`)}</p>}<div className="tm-gate-next">{canRequest ? <Action page="request" event={event.id}>{t('게스트 신청', 'Guest request')}</Action> : <p>{event.status === 'ARCHIVED' || event.status === 'LIVE' ? t('이 이벤트의 온라인 신청은 마감되었습니다.', 'Online requests for this event are closed.') : t('현재 온라인 신청 기간이 아닙니다.', 'Online requests are not open.')}</p>}<Action page="lineup" event={event.id} secondary>{t('라인업 보기', 'Explore lineup')}</Action></div></section>
    </div>
  </>;
}
