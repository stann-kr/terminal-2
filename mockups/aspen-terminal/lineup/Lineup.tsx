...
export function Lineup(props: ScreenProps & { artistId: string | null }) {
  ...
  useReadoutMotion(profileRef, { key: `${event?.id}:${artistId}:${lang}`, titles: '#tm-artist-name', content: '.tm-profile-top .tm-eyebrow,.tm-artist-data > div,.tm-artist-bio > p,#tm-artist-profile > p', rules: '.tm-artist-data' });
  ...
  return <>
    ...
    <div className="tm-lineup" data-has-profile={Boolean(selected)}>
      ...
      <section ref={profileRef} id="tm-artist-profile" className="tm-artist-profile tm-cell" onKeyDown={e => { if (e.key === 'Escape') returnToRoster(); }} aria-labelledby="tm-artist-name">
        {selected ? <><div className="tm-profile-top"><p className="tm-eyebrow">ARTIST / {selected.id}</p><button type="button" className="tm-button tm-roster-return" onClick={returnToRoster}>{t('명단으로', 'Back to list')}</button></div><h2 id="tm-artist-name" ref={headingRef} tabIndex={-1}>{selected.name}</h2><dl className="tm-artist-data tm-motion-rule"><div><dt>{t('출신', 'Origin')}</dt><dd>{selected.origin}</dd></div><div><dt>{t('스테이지', 'Stage')}</dt><dd>{selected.dock}</dd></div><div><dt>{t('출연 시간', 'Set time')}</dt><dd>{selected.time === 'TBA' ? t('미공개', 'Not announced') : selected.time}</dd></div></dl><div className="tm-artist-bio tm-prose">{biography(selected, lang).map((line, i) => <p key={i}>{line}</p>)}</div></> : <><h2 id="tm-artist-name" tabIndex={-1} ref={headingRef}>{artistId ? t(requested ? '아티스트 정보 미공개' : '아티스트를 찾을 수 없습니다.', requested ? 'Artist not published' : 'Artist not found.') : t('공개된 아티스트가 없습니다.', 'No published artists.')}</h2><p role="status">{t('공개 명단에서 아티스트를 선택해 주세요.', 'Choose an artist from the published roster.')}</p></>}
      </section>
    </div>
  </>;
}
