...
export function Home(props: ScreenProps & { poster: string }) {
  ...
  return <article className="tm-home" data-poster={showPoster}>
    <div className="tm-home-title tm-cell tm-motion-rule"><div data-motion-copy className="tm-home-meta"><EventState event={event} t={t} /><span className="tm-eyebrow">{event.id}</span></div><h1 data-motion-title tabIndex={-1}>{title}</h1><p className="tm-eyebrow">SEOUL / TECHNO</p></div>
...
export function Gate(props: ScreenProps & { poster: string }) {
  ...
  return <>
    ...
    <div className="tm-gate-grid">
      <section className="tm-gate-copy tm-cell"><div className="tm-gate-meta"><EventState event={event} t={t} /><span>{event.id}</span></div><h2 data-motion-title>{event.subtitle}</h2><div className="tm-prose">{event.description?.[lang].split('\n\n').map((text, i) => <p data-motion-copy key={i}>{text}</p>)}</div>{props.poster && props.poster !== failedPoster && <img className="tm-gate-poster" src={props.poster} alt={`${event.session} ${t('포스터', 'poster')}`} onError={() => setFailedPoster(props.poster)} />}</section>
      <section className="tm-gate-details tm-cell"><h2 className="tm-eyebrow">{t('이벤트 정보', 'EVENT INFO')}</h2><dl>{details.map(([label, value]) => <div data-motion-copy key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></section>
