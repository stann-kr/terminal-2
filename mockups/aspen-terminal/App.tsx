...
export default function App() {
  ...
  return <MotionProvider crt={crt}><div className="tm-preview">
    ...
    <Shell page={route.page} eventId={event?.id} viewKey={route.page === 'entry' ? `entry:${route.entryMode}:${entryRun}` : `${route.page}:${errorPage ? 'error' : 'ready'}`} motionKey={`${route.page}:${route.page === 'lineup' ? '' : event?.id}:${scenario}:${lang}:${poster}:${posterError}`} lang={lang} t={t} setLang={setLang} crt={crt} toggleCrt={() => setCrt(value => !value)}>
      ...
      <div hidden={route.page !== 'transmit'} data-active={route.page === 'transmit'}><Transmit t={t} lang={lang} failSubmission={failSubmission} samples={samples} active={route.page === 'transmit'} /></div>
