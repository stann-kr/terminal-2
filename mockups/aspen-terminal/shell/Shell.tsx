...
export function Shell({ page, lang, t, setLang, children }: { page: Page; lang: Lang; t: Translate; setLang: (lang: Lang) => void; children: ReactNode }) {
  ...
  return <div className="tm-shell" data-crt={crt}>
    ...
    <header className="tm-header" onKeyDown={e => { if (e.key === 'Escape' && menu) { setMenu(false); menuRef.current?.focus(); } }}>
      <div className="tm-topline"><a id="tm-brand" className="tm-brand" href={href('home')}>TERMINAL<span>{' // SEOUL'}</span></a><div className="tm-utilities"><a className="tm-channel-link" href={href('link')}>{t('공식 채널', 'Channels')}</a><button type="button" aria-label={t('CRT 화면 효과', 'CRT display effects')} aria-pressed={crt} onClick={() => setCrt(!crt)}>CRT <span aria-hidden="true">{crt ? '■' : '□'}</span></button><button type="button" aria-label={t('영어로 보기', 'Switch to Korean')} onClick={() => setLang(lang === 'ko' ? 'en' : 'ko')}>{lang === 'ko' ? 'EN' : 'KO'}</button><button ref={menuRef} className="tm-menu-toggle" type="button" aria-expanded={menu} aria-controls="tm-navigation" onClick={() => setMenu(!menu)}>{t('메뉴', 'Menu')} [{menu ? '−' : '+'}]</button></div></div>
