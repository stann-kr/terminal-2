...
export function Status({ events, t }: ScreenProps) {
  ...
  return <><PageHeading code="STATUS / EVENT HISTORY" title={t('이벤트 기록', 'Event history')} />{years.length ? years.map(year => <section key={year} className="tm-history-year"><h2 className="tm-cell">{year}</h2><div>{archived.filter(event => event.date.startsWith(year)).map(event => <a key={event.id} href={href('gate', event.id)} className="tm-history-entry"><div><span className="tm-eyebrow">{event.id}</span><h3>{event.session}</h3><p>{event.subtitle}</p></div><div><p>{event.date}</p><p>{event.venue}</p></div><div><span className="tm-eyebrow">LINEUP</span><p>{event.artists.filter(isPublicArtist).map(artist => artist.name).join(' / ')}</p></div></a>)}</div></section>) : <p className="tm-cell">{t('기록된 이벤트가 없습니다.', 'No event history.')}</p>}</>;
}
