...
export function Status({ events, t }: ScreenProps) {
  ...
  return <><PageHeading code="STATUS / EVENT HISTORY" title={t('이벤트 기록', 'Event history')} />{years.length ? years.map(year => <section key={year} className="tm-history-year"><h2 data-motion-title className="tm-cell"><TerminalText>{year}</TerminalText></h2><div>{archived.filter(event => event.date.startsWith(year)).map(event => <a key={event.id} href={href('gate', event.id)} className="tm-history-entry"><div data-motion-copy><span className="tm-eyebrow">{event.id}</span><h3>{event.session}</h3><p>{event.subtitle}</p></div><div><p>{event.date}</p><p>{event.venue}</p></div><div><span className="tm-eyebrow">LINEUP</span><p>{event.artists.filter(isPublicArtist).map(artist => artist.name).join(' / ')}</p></div></a>)}</div></section>) : <p className="tm-cell">{t('기록된 이벤트가 없습니다.', 'No event history.')}</p>}</>;
}

export function About({ lang, t }: ScreenProps) {
  const manifesto = lang === 'ko' ? manifestoKo : manifestoEn;
  return <><PageHeading code="ABOUT / TERMINAL" title={t('TERMINAL 소개', 'About TERMINAL')} /><div className="tm-about"><section className="tm-about-identity tm-cell"><p className="tm-eyebrow">SEOUL / TECHNO PLATFORM</p><p data-motion-title className="tm-about-wordmark" aria-hidden="true">TER<br />MINAL</p><h2 data-motion-copy>{manifesto[0]}</h2></section><section className="tm-about-copy tm-cell"><div className="tm-prose">{manifesto.slice(1).map((line, i) => <p data-motion-copy key={i}>{line}</p>)}</div><div className="tm-action-group"><Action page="gate">{t('이벤트 보기', 'Explore events')}</Action><Action page="link" secondary>{t('공식 채널', 'Official channels')}</Action><a className="tm-text-link" href={href('entry')}><span>{t('터미널 체험', 'Terminal experience')}</span></a></div></section></div></>;
}
