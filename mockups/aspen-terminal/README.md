...
- 부팅·IDLE에는 헤더/메뉴/OS 푸터/목업 검토 도구를 표시하지 않습니다. ENTER 또는 이벤트 바로 보기로 진입한 뒤 나타나며, 언어와 CRT 설정은 진입 후 변경할 수 있습니다.
- 같은 CRT 프레임에서 전원 표현 → 콘솔 출력·언어 자동 감지 → **ENTER TERMINAL** → 홈으로 이어집니다. 언어 선택을 기다리는 단계는 없습니다. 부팅 출력은 시각 연출이며 서버 연결 검사를 뜻하지 않습니다.
...
- **애니메이션 건너뛰기**는 즉시 ENTER 단계로 이동하고 **이벤트 바로 보기**는 부팅 완료 기록 없이 홈으로 이동합니다. 기존 홈/상세 직접 URL도 유지됩니다.
