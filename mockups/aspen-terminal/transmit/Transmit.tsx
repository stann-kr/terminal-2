...
export function Transmit({ t, failSubmission, samples, active }: Pick<ScreenProps, 't'> & { failSubmission: boolean; samples: boolean; active: boolean }) {
  ...
  useReadoutMotion(logRef, { key: `${currentPage}:${samples}:${entries[0]?.id}`, active, content: 'li > div,li > p,.tm-log-empty' });
  useReadoutMotion(formRef, { key: `${error}:${sent}`, active, content: '.tm-field-error,[role=status]' });
