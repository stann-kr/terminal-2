...
export default function TerminalPanel({ children, className = '', bodyClassName = '', title, accent = 'primary', headingLevel = 2 }: TerminalPanelProps) {
  ...
  useGSAP(() => {
    ...
    gsap.from('[data-panel="line"]', { scaleX: 0, duration: 0.18, ease: 'steps(6)', scrollTrigger: { trigger: rootRef.current, scroller: rootRef.current?.closest<HTMLElement>('[data-scroll-region]') ?? undefined, start: 'top 96%', once: true } });
