...
function DirectoryItem({ item, current, onNavigate }: {
  item: typeof DIRECTORY[number]; current: boolean; onNavigate: () => void;
}) {
  const { lang } = useLang();
  ...
  return <li className={styles.item}>
    <Link ref={ref} href={item.href} onClick={onNavigate} aria-current={current ? 'page' : undefined} className={styles.link}>
      ...
      <span data-control-label className={styles.name}>{item[lang]}</span>
    </Link>
  </li>;
}
  ...
  return <header className={styles.header} onKeyDown={event => {
    ...
    <div className={styles.rail}>
      ...
      <div className={styles.utilities}>
        <Link href="/link" className={styles.channels} aria-current={pathname === '/link' ? 'page' : undefined}>{lang === 'ko' ? '공식 채널' : 'Official channels'}</Link>
    ...
    <nav data-shell="navigation" aria-label={lang === 'ko' ? '주요 메뉴' : 'Main navigation'} className={styles.navigation}>
      <ul className={styles.core}>{DIRECTORY.slice(0, 3).map(item => <DirectoryItem key={item.href} item={item} current={current?.href === item.href} onNavigate={() => closeMenu(item.href)} />)}</ul>
      <ul ref={secondaryRef} id={secondaryId} className={styles.secondary} data-open={isOpen}>
        {DIRECTORY.slice(3).map(item => <DirectoryItem key={item.href} item={item} current={current?.href === item.href} onNavigate={() => closeMenu(item.href)} />)}
        <li className={styles.mobileChannels}><Link href="/link" onClick={() => closeMenu('/link')} aria-current={pathname === '/link' ? 'page' : undefined}>{lang === 'ko' ? '공식 채널' : 'Official channels'}</Link></li>
      </ul>
    </nav>
